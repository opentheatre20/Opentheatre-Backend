...
router.get('/google/callback', passport.authenticate('google', { session: false, failureRedirect: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/login?error=google_failed` }), (req, res) => {
  ...
  const accessToken = jwt.sign({ id: user._id, role: user.role }, (process.env.JWT_SECRET || 'secret') as jwt.Secret, { expiresIn: '30d' } as jwt.SignOptions);
  
  const refreshToken = jwt.sign({ id: user._id, role: user.role }, (process.env.JWT_REFRESH_SECRET || 'refresh_secret') as jwt.Secret, { expiresIn: '30d' } as jwt.SignOptions);
